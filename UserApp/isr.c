...
void TIM7_IRQHandler (void)
{
    ...
    if (stallFlag == 1 || parallelFlag == 1)
    {
        ...
        if (stallFlag == 1)
        {
            pwm_set_duty(TIM2_PWM_CH1_A15, GetServoDuty(-14));
            stallFlag = 2;
        } else if (parallelFlag == 1)
        {
            pwm_set_duty(TIM2_PWM_CH1_A15, GetServoDuty(-14));
            parallelFlag = 2;
        }
    ...
    if (stallFlag == 3 || parallelFlag == 4)
    {
        ...
        if(timeCount < 1200)
        {
            ...
            if (timeCount < 110)
            {
                if (stallFlag == 3)
                {
                    pwm_set_duty(MOTOR_L_PIN, 1100);
                    pwm_set_duty(MOTOR_R_PIN, 1100);
                } else if (parallelFlag == 4)
                {
                    if (timeCount < 40)
                    {
                        pwm_set_duty(MOTOR_L_PIN, 3800);
                        pwm_set_duty(MOTOR_R_PIN, 3800);
                    } else
                    {
                        pwm_set_duty(MOTOR_L_PIN, 0);
                        pwm_set_duty(MOTOR_R_PIN, 0);
